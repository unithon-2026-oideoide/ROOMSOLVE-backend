      ...
      schemas: {
        ...
        Report: {
          ...
          properties: {
            ...
            available_times: {
              type: 'string',
              nullable: true,
              description: '세입자가 집에 있는 시간대. 자유 텍스트. db/009',
              example: '평일 오후, 주말 오전',
            },
        ...
        Quote: {
          ...
          properties: {
            ...
            status: { type: 'string', enum: ['recommended', 'selected', 'rejected'] },
            is_outlier: { type: 'boolean', description: 'DB 컬럼이지만 사용하지 않음 (항상 false)' },
            proposed_visit_at: {
              type: 'string',
              format: 'date-time',
              nullable: true,
              description: '업체가 제안한 방문 가능 시간. status가 selected로 바뀌는 순간 이 값으로 repair_schedule이 자동 생성된다. db/009',
            },
            created_at: { type: 'string', format: 'date-time' },
          },
        },
        QuoteWithOutlierInfo: {
          ...
          properties: {
            ...
            proposed_visit_at: {
              ...
              format: 'date-time',
              nullable: true,
              description: '업체가 제안한 방문 가능 시간. db/009',
            },
            created_at: { type: 'string', format: 'date-time' },
